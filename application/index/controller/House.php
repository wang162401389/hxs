...
class House extends Frontend
{
    ...
    protected $ad = 'house_loan_index_figure';
    ...
    {
        $list = Loan::with('category')->where('category.type', 'house_loan')->order('create_time desc')->limit(5)->select();
        $this->view->assign('list', $list);
        
        $article_list = [];
        $cat_id = \think\Db::name('category_setting')->where('name', 'show_in_house_index')->value('value');
        if ($cat_id > 0) 
        {
        ...
        $bottom_pic = $this->loadad('house_index_bottom');
    ...
    public function more()
    {
        if ($this->request->isAjax() && $this->request->isPost())
        {
            ...
            $count = Loan::with('category')->where('category.type', 'house_loan')->count();

            $this->code = 0;
            if ($count) {
                $list = Loan::with('category')->where('category.type', 'house_loan')->limit(config('paginate.list_rows'))->page($page)->order('create_time desc')->select();
        ...
        $bottom_pic = $this->loadad('house_loan_more');
        $this->view->assign('botttom_pic', $bottom_pic);
    ...
    public function detail($id = NULL)
    {
        ...
        $cat_arr = Category::getCategoryArray('house_loan', 'normal', null, 'id,type');
        ...
        $article_list = [];
        $cat_id = \think\Db::name('category_setting')->where('name', 'show_in_house_detail')->value('value');
        if ($cat_id > 0)
        {
            $cat_name = Category::get($cat_id);
            $this->view->assign('mod_name', $cat_name->name);
