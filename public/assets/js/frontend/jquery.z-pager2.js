	...
	var methods = {
		pageInit: function(options){
			...
			return $(this).each(function(k,v){
				...
				methods.pageData(_v, opts.current, opts.hrefId);
			})
		},
		pageData: function(_v, _current, _hrefId){
			/**
			 * [opts this plug propertys]
			 * @type {Obeject}
			 */
			...
			if(opts.ajaxSetData&&(typeof(opts.ajaxSetData)==='boolean')){
				if(opts.url!=='' && typeof(opts.url)==='string'){
					ajaxOpts = methods.ajaxData(opts.url, _current, _hrefId);
		...
		dataRender: function(_v, _data){
			...
			for(var i=0;i<_data.length;i++){
				var oDate = new Date(_data[i].createtime * 1000),  
				...
				cells += '<li><a href="/strategy/'+ _data[i].id +'.html" target="_blank">'+ _data[i].title +'<span class="fr">'+ oTime +'</span></a></li>';
			}
			...
			var _a = _v.find("a");
				$.each(_a,function(index,item){
					...
					_this.on("click",function(){
						...
						methods.pageData(_v, _p, o.hrefId);
						$('.pg-prev').text("上一页");
						$('.pg-next').text("下一页");
					})
				})
		...
		ajaxData: function(_url, _current, _hrefId){
			/**
