	...
	var methods = {
		pageInit: function(options){
			...
			return $(this).each(function(k,v){
				...
				methods.pageData(_v, opts.current, opts.limit_order, opts.deadline_order, opts.limit, opts.deadline,opts.type,opts.profession);
			})
		},
		pageData: function(_v, _current, _limit_order, _deadline_order, _limit, _deadline, _type, _profession){
			/**
			 * [opts this plug propertys]
			 * @type {Obeject}
			 */
			...
			if(opts.ajaxSetData&&(typeof(opts.ajaxSetData)==='boolean')){
				if(opts.url!=='' && typeof(opts.url)==='string'){
					ajaxOpts = methods.ajaxData(opts.url, _current, _limit_order, _deadline_order,_limit,_deadline,_type, _profession);
		...
		dataRender: function(_v, _data){
			...
			var cells = '';
				for(var i=0;i<_data.length;i++){
					cells += '<div class="item"><div class="blank_logo fl"><img src="' + _data[i].picurl + '" class="p_img"></div><div class="txt fl"> <div class="clearfix txt_top"><strong class="fl red span1 overflow_text">' +  _data[i].title + '</strong></div><div class="txt_btm clearfix"><span>贷款金额<br /><strong class="red limitdata">' + _data[i].limit_text + '</strong></span><span>周期<br /><strong class="red">' + _data[i].deadline_text + '</strong></span> <span>月息<br /><strong class="red">' + _data[i].interest + '</strong></span><span>放款速度<br /><strong class="red">' + _data[i].loan_time + '</strong></span></div></div> <a href="credit/' + _data[i].id + '.html" target="_blank" class="apply_btn fl">申请 </a></div>';
				}
			...
			var _a = _v.find("a");
				$.each(_a,function(index,item){
					...
					_this.on("click",function(){
						...
						methods.pageData(_v, _p, o.limit_order, o.deadline_order, o.limit, o.deadline, o.type, o.profession);
						$('.pg-prev').text("上一页");
						$('.pg-next').text("下一页");
					})
				})
		...
		ajaxData: function(_url, _current, _limit_order, _deadline_order, _limit, _deadline, _type, _profession){
			/**
			...
			return (function(){
				...
				$.ajax({
					...
					data: {
						...
						"deadline_order": _deadline_order,
						"limit": _limit,
						"deadline": _deadline,
						"type": _type,
	                ...
	                success: function(data) {
	                	if(data.data != null){
		                	...
		                	if(data.data.total && (data.data.total!==0)){
		                        ...
		                        parms['rows'] = data.data.row;
		                	}else{
		                		$.pageError(3);
		                	}
		                }else{
		                	$('.par_list').html("<p style=' text-align: center; margin-top: 200px;'>没有数据哦！</p>")
		                }
	                },
