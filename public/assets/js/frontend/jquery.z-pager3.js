	...
	var methods = {
		pageInit: function(options){
			...
			return $(this).each(function(k,v){
				...
				methods.pageData(_v, opts.current, opts.title);
			})
		},
		pageData: function(_v, _current, _title){
			/**
			 * [opts this plug propertys]
			 * @type {Obeject}
			 */
			...
			if(opts.ajaxSetData&&(typeof(opts.ajaxSetData)==='boolean')){
				if(opts.url!=='' && typeof(opts.url)==='string'){
					ajaxOpts = methods.ajaxData(opts.url, _current, _title);
		...
		dataRender: function(_v, _data){
			...
			for(var i=0;i<_data.length;i++){
				var oDate = new Date(_data[i].create_time * 1000),  
		        ...
		        var a = '';
		        if(_data[i].reply_list[0].mb_content != _data[i].reply_list[0].content){
		        	a = '<a href="ask/'+ _data[i].id +'.html" class="go_all">[阅读全文]</a>';
		        }

				//cells += '<div class="item clearfix"> <p class="question"><span>' + _data[i].title + '</span> <span class="time fr">' + oTime + '</span></p><p class="anwser">' + _data[i].reply_list[0].mb_content + ' <a href="ask/'+ _data[i].id +'.html" class="go_all">[阅读全文]</a></p><p class="anwser_num"><a href="ask/'+ _data[i].id +'.html" class="go_all">● ' + _data[i].reply_count + '个回答</a></p></div>';

				cells += '<div class="item clearfix"> <p class="question"><span><a href="ask/'+ _data[i].id +'.html">' + _data[i].title + '</a></span> <span class="time fr">' + oTime + '</span></p><p class="anwser">' + _data[i].reply_list[0].mb_content + a +'</p><p class="anwser_num"><a href="ask/'+ _data[i].id +'.html" class="go_all">● ' + _data[i].reply_count + '个回答</a></p></div>';
			...
			var _a = _v.find("a");
				$.each(_a,function(index,item){
					...
					_this.on("click",function(){
						...
						methods.pageData(_v, _p, o.title);
						$('.pg-prev').text("上一页");
						$('.pg-next').text("下一页");
					})
				})
		...
		ajaxData: function(_url, _current, _title){
			/**
			...
			return (function(){
				...
				$.ajax({
					url: _url,
					type: 'post',
					data: {"page":_current, "title": _title},
	                dataType: 'json',
	                cache : false,  
        			async : false,
	                success: function(data) {
	                	if(data.data.total && (data.data.total!= 0)){
	                		...
	                		$('.list_con').html("<p style='text-align:center;    margin-top: 150px;'>很抱歉！找不到您搜索的内容!</p>");
	                		//$.pageError(3);
	                	}
